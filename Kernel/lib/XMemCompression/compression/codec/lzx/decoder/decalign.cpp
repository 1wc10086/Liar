    ...
    while (window_pos < end_pos)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (extra_bits != 3)
                {
                    extra = bitbuf >> (35 - extra_bits);
                    bitcount += 3 - static_cast<int>(extra_bits);
                    bitbuf <<= (extra_bits - 3);
                    if (bitcount <= 0)
                    {
                        const unsigned int next = static_cast<unsigned int>(input_curpos[0]) |
                ...
                unsigned int match_base = static_cast<unsigned int>(
                    context->MP_POS_minus2_table[static_cast<unsigned char>(offset_slot)]) + 8 * extra;
                const unsigned int aligned_symbol =
                    static_cast<unsigned char>(context->dec_aligned_table[bitbuf >> 25]);
                const unsigned int aligned_len = context->dec_aligned_len[aligned_symbol];
                bitcount -= static_cast<int>(aligned_len);
                bitbuf <<= aligned_len;
                if (bitcount <= 0)
                {
                    const unsigned int next = static_cast<unsigned int>(input_curpos[0]) |
        ...
        const unsigned int window_mask = context->dec_window_mask;
        ...
        do
        {
            const unsigned char value =
                mem_window[window_mask & (write_pos - static_cast<int>(match_offset))];
    ...
    while (window_pos < end_pos)
    {
        ...
        else
        {
            ...
            if (extra_bits < 3)
            {
                ...
                {
                    match_offset = static_cast<unsigned int>(
                        context->MP_POS_minus2_table[static_cast<unsigned char>(offset_slot)]);
                }
            }
            else
            {
...
int decode_aligned_offset_block(t_decoder_context* context, long bufpos, int amount_to_decode)
{
    ...
    if (current_bufpos < 0x101)
    {
        ...
        const int new_bufpos = static_cast<int>(
            special_decode_aligned_block(context, current_bufpos, special_amount));
    ...
    return static_cast<int>(fast_decode_aligned_offset_block(context, current_bufpos, remaining));
}
}
