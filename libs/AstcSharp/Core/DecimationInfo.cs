...
internal sealed class DecimationInfo
{
    ...
    public DecimationInfo(int texelCount, int[] weightIndices, int[] weightFactors)
    {
        ...
        var gridSize = weightIndices.Max() + 1;
        var totals = new double[gridSize];
        for (var i = 0; i < weightIndices.Length; i++)
        {
            totals[weightIndices[i]] += weightFactors[i];
        }
    ...
    public ReadOnlyMemory<double> FactorTotals { get; }
