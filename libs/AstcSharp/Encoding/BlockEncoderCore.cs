...
internal static partial class BlockEncoderCore
{
    ...
    private const int MaxSearchedPartitions = 1;
    ...
    private const int SeedFinalists = 1;
        ...
        where TStrategy : struct, IColorSpaceStrategy<TTexel>
    {
        ...
        if (FastSearchPolicy.EnableDualPlane)
        {
        ...
        where TStrategy : struct, IColorSpaceStrategy<TTexel>
    {
        ...
        long earlyOutError = strategy.EarlyOutPerSampleError * texelCount * ChannelCount;
        ...
        if (SearchConfigs<TTexel, TStrategy>(in block, candidateModes[..modeCount], ColorStartBit, earlyOutError, in scratch) is not { } result)
        {
        ...
        where TStrategy : struct, IColorSpaceStrategy<TTexel>
    {
        ...
        var cache = new EndpointPreparationCache(stackalloc int[256], stackalloc int[EndpointPreparationCache.Capacity]);
        foreach (ColorEndpointMode mode in candidateModes)
        {
            ...
            cache.Clear();
            foreach (var grid in EncodingConfigTable.Get(block.Footprint, colorValueCount, colorStartBit, false))
            {
                ...
                foreach (var config in grid.Configurations)
                {
                    ...
                    {
                        PrepareConfig<TTexel, TStrategy>(in block, mode, grid.WeightCount, grid.Decimation, colorRange, in scratch, ref cache);
                        preparedColorRange = colorRange;
                    }

                    if (default(TStrategy).TryEndpointRefinement)
                    {
                        cache.Restore(colorRange, scratch.CandidateColorValues[..colorValueCount],
                            scratch.EffectiveLow[..(ChannelCount * block.PartitionCount)],
                            scratch.EffectiveHigh[..(ChannelCount * block.PartitionCount)], scratch.IdealWeights[..block.Texels.Length]);
                    }

                    long error = MeasureConfig<TTexel, TStrategy>(in block, mode, grid.WeightCount, weightRange, colorRange, grid.Decimation, in scratch, bestError);
                    if (error < bestError)
                    {
                        bestError = error;
                        best = new BestConfig(mode, grid.Width, grid.Height, weightRange, colorRange, colorValueCount);
                        scratch.CandidateColorValues[..colorValueCount].CopyTo(scratch.BestColorValues);
                        scratch.CandidateGridWeights[..grid.WeightCount].CopyTo(scratch.BestGridWeights);

                        if (bestError <= earlyOutError)
                        {
                            return new SearchResult<BestConfig>(best, bestError);
                        }
                    }
                }
            }
