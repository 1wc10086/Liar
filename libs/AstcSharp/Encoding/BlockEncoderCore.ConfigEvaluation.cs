...
internal static partial class BlockEncoderCore
{
    ...
    private static void PrepareConfig<TTexel, TStrategy>(
        ...
        scoped ref EndpointPreparationCache cache)
        ...
        where TStrategy : struct, IColorSpaceStrategy<TTexel>
    {
        ...
        var cachedColors = scratch.CandidateColorValues[..(valuesPerPartition * block.PartitionCount)];
        var cachedLow = effectiveLow[..(ChannelCount * block.PartitionCount)];
        var cachedHigh = effectiveHigh[..(ChannelCount * block.PartitionCount)];
        var cachedWeights = scratch.IdealWeights[..block.Texels.Length];
        if (cache.Restore(colorRange, cachedColors, cachedLow, cachedHigh, cachedWeights))
        {
            DecimationFit.Fit(cachedWeights, decimation, gridWeightCount, scratch.FittedGrid[..gridWeightCount]);
            return;
        }
        ...
        cache.Store(colorRange, cachedColors, cachedLow, cachedHigh, cachedWeights);
        ...
        where TStrategy : struct, IColorSpaceStrategy<TTexel>
    {
        ...
        for (int t = 0; t < texels.Length; t++)
        {
            ...
            if (!strategy.TryEndpointRefinement && error >= bestError)
            {
                return error;
            }
        }
