...
internal static class ColorGeometry
{
    ...
    {
        var quantized = Quantization.GetQuantizedWeights(weightRange);
        var effective = Quantization.GetEffectiveWeights(weightRange);
        for (int i = 0; i < fittedGrid.Length; i++)
        {
            int quant = quantized[RoundWeight(fittedGrid[i])];
            quantGridWeights[i] = quant;
            effectiveGrid[i] = effective[quant];
        }
    }

    private static int RoundWeight(double weight) => (int)(weight + 0.5);
}
