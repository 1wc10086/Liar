...
{
    internal static int special_decode_verbatim_block(t_decoder_context context, int bufpos, int amount_to_decode)
    {
        ...
        while (window_pos < end_pos)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (bitcount <= 0)
                    {
                        ...
                        if (bitcount <= 0)
                        {
                            uint more = (uint)context.dec_input[input_curpos] |
                                        ((uint)context.dec_input[input_curpos + 1] << 8);
                            input_curpos += 2;
            ...
            do
            {
                byte value = mem_window[context.dec_window_mask & (uint)(write_pos - (int)match_offset)];
                mem_window[window_pos] = value;
                if (window_pos < 0x101)
                {
                    mem_window[write_pos + (int)context.dec_window_size] = value;
    ...
    internal static int fast_decode_verbatim_block(t_decoder_context context, int bufpos, int amount_to_decode)
    {
        int bitcount = context.dec_bitcount;
        uint bitbuf = context.dec_bitbuf;
        int input_curpos = context.dec_input_curpos;
        ...
        while (window_pos < end_pos)
        {
            ...
            else
            {
                if (offset_slot <= 3)
                {
                    match_offset = (uint)context.MP_POS_minus2_table[3];
                }
                else
                {
                    uint extra_bits = context.dec_extra_bits_table[(byte)offset_slot];
    ...
    internal static int decode_verbatim_block(t_decoder_context context, int bufpos, int amount_to_decode)
    {
        ...
        if (current_bufpos < 0x101)
        {
            ...
            int new_bufpos = special_decode_verbatim_block(context, current_bufpos, special_amount);
        ...
        return fast_decode_verbatim_block(context, current_bufpos, remaining);
    }
}
